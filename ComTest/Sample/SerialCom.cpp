// com.cpp: implementation of the CSerialCom class.
//
//////////////////////////////////////////////////////////////////////
...
#include "COM32.h"
CSerialCom::CSerialCom(HWND hWnd)
{
  ReadThread = NULL;
  WriteThread = NULL;
  hCommFile = 0;
...
bool CSerialCom::WriteCommData(char * pDataToWrite, WORD dwSizeofDataToWrite)
{
	char *Buffer;
	if ((WriteThread != NULL) && (dwSizeofDataToWrite != 0))
	{
		...
		if (PostThreadMessage(WriteThread->threadID, PWM_COMMWRITE,WPARAM(dwSizeofDataToWrite), LPARAM(Buffer) ))
		{
...
DWORD   CSerialCom::GetModemState()
{
	DWORD dwModemState;
...
bool CSerialCom::IsOpened()
{
	return Opened;
}
void CSerialCom::CloseReadThread()
{
	// If it exists...
	if (ReadThread !=NULL)
	{
...
void CSerialCom::CloseWriteThread()
{
	// If it exists...
	if (WriteThread != NULL)
	{
...
void CSerialCom::_SetCommTimeout()
{
...
void CSerialCom::SetBaudRate(DWORD Rate)
{
	if (Rate == FBaudRate )
        return;
...
void CSerialCom::SetParityCheck(bool b )
{
	if (b == FParityCheck )
        return;
...
void CSerialCom::SetOutx_CtsFlow(bool b)
{
	if (b == FOutx_CtsFlow )
        return;
...
void CSerialCom::SetOutx_DsrFlow( bool b )
{
	if (b == FOutx_DsrFlow )
        return;
...
void CSerialCom::SetDtrControl( DtrControl c )
{
	if (c == FDtrControl )
        return;
...
void CSerialCom::SetDsrSensitivity( bool b )
{
...
void CSerialCom::SetTxContinueOnXoff( bool b )
{
...
void CSerialCom::SetOutx_XonXoffFlow( bool b )
{
...
void CSerialCom::SetInx_XonXoffFlow( bool b )
{
...
void CSerialCom::SetReplaceWhenParityError( bool b )
{
...
void CSerialCom::SetIgnoreNullChar( bool b )
{
...
void CSerialCom::SetRtsControl( RtsControl c )
{
	if (c == FRtsControl )
        return;
...
void CSerialCom::SetXonLimit( WORD Limit )
{
	if (Limit == FXonLimit )
        return;
...
void CSerialCom::SetXoffLimit( WORD Limit )
{
	if (Limit == FXoffLimit )
        return;
...
void CSerialCom::SetByteSize(ByteSize Size)
{
	if (Size == FByteSize )
        return;
...
void CSerialCom::SetParity(Parity p)
{
	if (p == FParity )
        return;
...
void CSerialCom::SetStopBits(StopBits Bits)
{
	if (Bits == FStopBits )
        return;
...
void CSerialCom::SetXonChar(char c )
{
	if (c == FXonChar )
        return;
...
void CSerialCom::SetXoffChar(char  c )
{
	if (c == FXoffChar )
        return;
...
void CSerialCom::SetReplacedChar(char c)
{
	if (c == FReplacedChar )
        return;
...
void CSerialCom::SetReadIntervalTimeout( DWORD v)
{
...
void CSerialCom::SetReadTotalTimeoutMultiplier( DWORD v )
{
...
void CSerialCom::SetReadTotalTimeoutConstant( DWORD v )
{
...
void CSerialCom::SetWriteTotalTimeoutMultiplier(DWORD v )
{
...
void CSerialCom::SetWriteTotalTimeoutConstant( DWORD v )
{
...
void CSerialCom::SetCommName(LPCTSTR lpCommName)
{
	
	FCommName=lpCommName;
}

CString CSerialCom::GetCommName()
{
	return FCommName;
}
